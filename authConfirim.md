...
| supportAlg | 字符串&lt;32&gt; | 是 | 鉴权签名。 根据服务器返回的鉴权种子（/authen/apply接口返回的data参数），通过某个算法（MD5加密）生成的。即supportAlg=MD5\(data\)。应该是32长度的字符串。这个参数和鉴权申请接口的supportAlgs参数没有任何关系，只是命名不规范而已 |
